...
export function HoursEfficiencyChart({ chartData }: HoursEfficiencyChartProps) {
  const metrics = useMemo(() => {
    ...
    const totalEstimated = chartData.reduce(
      (acc, curr) => acc + curr.currentHrs,
      0,
    );
    const totalWorked = chartData.reduce(
      (acc, curr) => acc + curr.workedHrs,
      0,
    );
    ...
    const recentAvg =
      recent.length > 0
        ? recent.reduce((acc, d) => acc + d.efficiency, 0) / recent.length
        : 0;
    const olderAvg =
      older.length > 0
        ? older.reduce((acc, d) => acc + d.efficiency, 0) / older.length
        : recentAvg;
                    ...
                    if (name === 'workedHrs' && entry) {
                      ...
                      return (
                        <div className="flex flex-col gap-0.5">
                          ...
                          <span
                            className={`text-xs ${diff > 0 ? 'text-amber-600' : diff < 0 ? 'text-emerald-600' : 'text-muted-foreground'}`}
                          >
                            {diffLabel}
                          </span>
                        </div>
      ...
      {metrics && (
        <CardFooter className="text-xs text-muted-foreground pt-0">
          <div className="flex items-center justify-between w-full">
            <span>
              {metrics.variance >= 0 ? '+' : ''}
              {metrics.variance.toFixed(0)}h vs estimates
            </span>
            <span
