...
export default function CycleTimeChart({
  ...
  const metrics = useMemo(() => {
    ...
    const recentAvg =
      recent.length > 0
        ? recent.reduce((acc, d) => acc + d.avgCycleDays, 0) / recent.length
        : 0;
    const olderAvg =
      older.length > 0
        ? older.reduce((acc, d) => acc + d.avgCycleDays, 0) / older.length
        : recentAvg;
    ...
    const overallAvg =
      chartData.reduce((acc, d) => acc + d.avgCycleDays, 0) / chartData.length;

    // Health based on cycle time
    const health =
      currentAvg <= 5 ? 'fast' : currentAvg <= 14 ? 'normal' : 'slow';
        ...
        <div className="flex w-full items-center justify-between text-xs text-muted-foreground">
          <div className="flex items-center gap-4">
            <span className="flex items-center gap-1">
              <span
                ...
                style={{ backgroundColor: chartConfig.rollingAvg.color }}
              />
              Rolling
            </span>
            <span className="flex items-center gap-1">
              <span
                className="w-2 h-2 rounded-full"
                style={{ backgroundColor: chartConfig.avgCycleDays.color }}
              />
              Monthly
            </span>
          </div>
          {metrics && Math.abs(metrics.percentChange) > 5 && (
            ...
            >
              ...
              {metrics.percentChange.toFixed(0)}%
            </span>
          )}
        </div>
      </CardFooter>
